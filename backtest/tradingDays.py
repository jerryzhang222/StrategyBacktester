...
#    tdays = list(NYSE_tradingdays(datetime.datetime(yr,1,1),datetime.datetime(yr,12,31)))

def getTradeDateInts(startyear, startmonth, startday, endyear, endmonth, endday):
    tdays = list(NYSE_tradingdays(datetime.datetime(2014,1,1),datetime.datetime(2015,10,15)))
    tradeDateInts = []
    for day in tdays:
        tradeDateInts.append(day.strftime('%Y%m%d'))
    
    return(tradeDateInts)